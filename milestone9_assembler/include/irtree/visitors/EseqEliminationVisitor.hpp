...
class EseqEliminationVisitor: public TemplateVisitor<IRT::IrtStorage>,
  public std::enable_shared_from_this<EseqEliminationVisitor> {
 public:
  ...
  bool IsEseq(std::shared_ptr<BaseElement> element);
  void Rehang(std::shared_ptr<IRT::EseqExpression> element);
