...
class FrameEmulator: public std::enable_shared_from_this<FrameEmulator> {
 public:
  explicit FrameEmulator(FunctionType function_type);
  FrameEmulator() = default;
  ...
  std::shared_ptr<FrameEmulator> GetParent() const;
  void SetParent(std::shared_ptr<FrameEmulator> parent);
 ...
 private:
  std::shared_ptr<FrameEmulator> parent_ = nullptr;
